...
def get_move_type(move):  # return the type of the sorted move {'type':int, 'rank':int, 'len':int}
    move_size: int = len(move)
    move_dict: Dict[int, int] = collections.Counter(move)  # num of each cards
    ...
    count_dict: Dict[int, int] = collections.defaultdict(int)  # num of num in rank
    for c, n in move_dict.items():
        count_dict[n] += 1

    if move_size == 6:
        if len(move_dict) <= 3 and count_dict.get(4) == 1 and (count_dict.get(2) == 1 or count_dict.get(1) == 2):
