# return all moves that can beat rivals, moves and rival_move should be same type (move comparison)
import collections


def common_handle(moves, rival_move):  # rank comparison
    new_moves = list()
    for move in moves:
...
def filter_type_6_3_1(moves, rival_move):
    ...
    new_moves = list()
    for move in moves:
        # move.sort() # already sorted
        if move[1] > rival_rank:
            new_moves.append(move)
    return new_moves
